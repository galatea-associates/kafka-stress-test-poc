...
def process_data_config(config, server_args):
    ...
    for topic in config:
        producer_counters=Producer(init_val=config[topic]["Counter"]["Initial Value"], limit_val=config[topic]["Counter"]["Limit Value"], ready_start_prod=(not config[topic]['Load data first']))
